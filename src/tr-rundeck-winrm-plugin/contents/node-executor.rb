...
if ENV['RD_JOB_LOGLEVEL'] == "DEBUG"
   options[:command].sub!(/^(rm \-f|del) (\${env:TEMP}\/|C:\\WINDOWS\\TEMP\\)(.*)$/, '#Remove-Item -path \2\3')
else
   options[:command].sub!(/^(rm \-f|del) (\${env:TEMP}\/|C:\\WINDOWS\\TEMP\\)(.*)$/, 'Remove-Item -path \2\3')
end

options[:command].sub!(/^(Remove-Item -path )(\${env:TEMP}\/|C:\\WINDOWS\\TEMP\\)(.*\.)(sh|bat|ps1)$/, '#\1\2\3\4')
...
if ENV['RD_JOB_LOGLEVEL'] == "DEBUG"
   options[:command].sub!(/^(\${env:TEMP}\/.*\.)(sh|bat|ps1)$/, '$__SCRIPT__ = Get-Content \1\2 ; $__SCRIPT__ | Out-String | ' + ENV['RD_CONFIG_INVOCATION_STRING'] + ' - ;')
   $stderr.puts "TR Node Executor: Preserved script at remote host in its ${env:TEMP} . See lines above for path."
else
   options[:command].sub!(/^(\${env:TEMP}\/.*\.)(sh|bat|ps1)$/, '$__SCRIPT__ = Get-Content \1\2 ; Remove-Item -path \1\2 ; $__SCRIPT__ | Out-String | ' + ENV['RD_CONFIG_INVOCATION_STRING'] + ' - ;')
end
